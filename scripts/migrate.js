...
await sql`
  CREATE TABLE IF NOT EXISTS public.minutiae_migrations (
...
for (const name of files) {
  const existing = await sql`
    SELECT name FROM public.minutiae_migrations WHERE name = ${name}
  `
  ...
  await sql`
    INSERT INTO public.minutiae_migrations (name) VALUES (${name})
  `
